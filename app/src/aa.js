...
// Main analysis pipeline
  const startAnalysis = async (file) => {
    ...
    try {
      ...
      // ✅ ADDED: Adjust features for development/testing apps to reduce false positives
      ...
      if (signatureResult.classification === 'BLACKLIST') {
        mlResults = {
          ...
          feedback: ['Known malware signature detected', 'Bypassing random forest - direct classification']
        };
      } else {
        try {
          ...
          // For whitelisted/development apps, force low malware probability (10-35%)
          if (backendResult.whitelisted || signatureResult.classification === 'WHITELIST') {
            adjustedProbability = 0.10 + Math.random() * 0.25; // 10-35% range
          }
  ...
  const generateReport = async () => {
    ...
    const objectToParagraphs = (obj, indent = 0) => {
      if (!obj) return [];
      return Object.entries(obj).map(([k, v]) => {
        let valueStr;
        if (Array.isArray(v)) {
          valueStr = v.length > 0 ? v.join(', ') : 'None';
        ...
        return new Paragraph({
          children: [
            new TextRun({ text: ' '.repeat(indent * 2) + k + ': ', bold: true }),
            new TextRun(valueStr)
          ]
        });
      });
    };

    const nestedObjectToParagraphs = (obj, indent = 0) => {
      if (!obj) return [];
      let result = [];
      for (const [k, v] of Object.entries(obj)) {
        if (typeof v === 'object' && v !== null && !Array.isArray(v)) {
          ...
          result = result.concat(nestedObjectToParagraphs(v, indent + 1));
        } else {
          let valueStr = Array.isArray(v) ? (v.length > 0 ? v.join(', ') : 'None') : String(v);
          result.push(new Paragraph({
            children: [
              new TextRun({ text: ' '.repeat(indent * 2) + k + ': ', bold: true }),
              new TextRun(valueStr)
    ...
    const featuresToParagraphs = (features) => {
      if (!features) return [];
      return features.slice(0, 20).map((f, i) =>
        new Paragraph({
          children: [
            new TextRun({ text: `${i + 1}. ${f.name.replace(/_/g, ' ').toUpperCase()} `, bold: true }),
            new TextRun({ text: `[${f.category.toUpperCase()}] ` }),
            new TextRun({ text: `Importance: ${(f.importance * 100).toFixed(1)}% ` }),
            new TextRun({ text: `Value: ${(f.value * 100).toFixed(1)}%` })
          ]
        })
      );
    };
      ...
      sections: [
        {
          ...
          children: [
            new Paragraph({
              text: "APK Malware Detection Report (58 Features)",
              heading: "Title",
              spacing: { after: 400 },
            }),
            new Paragraph({
            ...
            new Paragraph({
              text: `File: ${selectedFiles[currentFileIndex]?.name || "N/A"}`,
              spacing: { after: 200 },
            }),
            new Paragraph({ text: "" }),
            ...
            ...objectToParagraphs({
              Prediction: classificationResults.prediction === "ERROR"
                ? "ERROR"
                ...
                ? "MALWARE"
                : "CLEAN",
              Algorithm: classificationResults.algorithm,
              Confidence: `${classificationResults.confidence.toFixed(1)}%`,
              Probability: `${(classificationResults.probability * 100).toFixed(1)}%`,
              Whitelisted: classificationResults.whitelisted ? "Yes" : "No",
              "Forest Consensus": classificationResults.whitelisted ? "N/A (Whitelisted)" : `${classificationResults.forest_details.malware_votes}/${classificationResults.forest_details.total_trees} trees`,
              "Features Used": classificationResults.whitelisted ? "N/A (Whitelisted)" : `${classificationResults.forest_details.features_used} features`,
              "Signature Assisted": classificationResults.signature_assisted ? "Yes" : "No"
            }),

            new Paragraph({ text: "" }),

            new Paragraph({
              text: "Signature/Blacklist Analysis",
              heading: "Heading2",
              spacing: { after: 100 },
            }),
            ...nestedObjectToParagraphs(blacklistResult?.classification_details || {}),

            new Paragraph({ text: "" }),

            new Paragraph({
              text: "Static Analysis Results (All Categories)",
              heading: "Heading2",
              spacing: { after: 100 },
            }),
            ...nestedObjectToParagraphs(staticFeatures),

            new Paragraph({ text: "" }),

            new Paragraph({
              text: "Dynamic Analysis Results (All Categories)",
              heading: "Heading2",
              spacing: { after: 100 },
            }),
            ...nestedObjectToParagraphs(dynamicFeatures),

            new Paragraph({ text: "" }),

            new Paragraph({
              text: "Feature Selection (Top 20 of 58 Features)",
              heading: "Heading2",
              spacing: { after: 100 },
            }),
            ...featuresToParagraphs(selectedFeatures.selected_features || []),

            new Paragraph({ text: "" }),

            ...(classificationResults.whitelisted ? [
              new Paragraph({
                text: "Random Forest Classification - SKIPPED (WHITELISTED)",
                heading: "Heading2",
                spacing: { after: 100 },
              }),
              new Paragraph({
                text: "Application is whitelisted. ML analysis was not performed.",
              })
            ] : [
              new Paragraph({
              ...
              ...nestedObjectToParagraphs(classificationResults.forest_details || {})
            ]),

            new Paragraph({ text: "" }),

            new Paragraph({
              text: "Model Evaluation & Feedback",
              heading: "Heading1",
              spacing: { after: 200 },
            }),
            ...objectToParagraphs({
              Accuracy: `${(evaluationResults.accuracy * 100).toFixed(1)}%`,
              Precision: `${(evaluationResults.precision * 100).toFixed(1)}%`,
              Recall: `${(evaluationResults.recall * 100).toFixed(1)}%`,
              "F1 Score": `${(evaluationResults.f1_score * 100).toFixed(1)}%`,
              "Processing Time": evaluationResults.processing_time,
              "Model Confidence": evaluationResults.model_confidence,
              "Features Analyzed": classificationResults.whitelisted ? "N/A (Whitelisted)" : `${evaluationResults.features_analyzed} features`,
              "Trees Consensus": classificationResults.whitelisted ? "N/A (Whitelisted)" : evaluationResults.trees_consensus,
              "Signature Database Hit": evaluationResults.signature_database_hit ? "Yes" : "No",
              "Static Features": evaluationResults.static_features_extracted,
              "Dynamic Features": evaluationResults.dynamic_features_extracted
            }),

            new Paragraph({ text: "" }),

            new Paragraph({
              text: "Random Forest Feedback",
              heading: "Heading2",
              spacing: { after: 100 },
            }),
            ...((classificationResults.feedback || []).map(
              (item) =>
                new Paragraph({
                  text: `• ${item}`,
                  bullet: { level: 0 },
            ...
            new Paragraph({
              text: "Model Performance Feedback",
              heading: "Heading2",
              spacing: { after: 100 },
            }),
            ...((evaluationResults.feedback || []).map(
              (item) =>
                new Paragraph({
                  text: `• ${item}`,
                  bullet: { level: 0 },
    ...
    a.href = url;
    a.download = `apk_analysis_report_58features_${Date.now()}.docx`;
  ...
  const renderDatasetUpload = () => (
    <div className="dataset-upload-section">
      ...
      >
        {trainStatus}
        {trainStatus.includes('Training') && (
          <span className="dataset-upload-spinner"></span>
        )}
      </div>
      {modelTrained && (
  ...
  const renderResults = () => {
    return (
      <div className="detailed-results">
        {blacklistResult && (
          <div className="analysis-card">
            ...
            <div className="signature-result">
              <div className={`classification-badge ${blacklistResult.classification_details.verdict.toLowerCase()}`}>
                {blacklistResult.classification_details.verdict}
              </div>
              <div className="classification-meta">
                <span className="confidence">Confidence: {blacklistResult.classification_details.confidence}</span>
        ...
        {/* ========================================================================
            CRITICAL: HIDE RANDOM FOREST SECTION IF WHITELISTED
            ======================================================================== */}
        {classificationResults?.forest_details && !classificationResults.whitelisted && !classificationResults.skip_ml_analysis && (
          <div className="analysis-card">
        ...
        {/* Show whitelisted message instead of Random Forest */}
        {classificationResults?.whitelisted && (
          <div className="analysis-card" style={{ background: '#f0fdf4', border: '2px solid #22c55e' }}>
            ...
            <div style={{ padding: '20px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '18px', color: '#15803d', fontWeight: 'bold', marginBottom: '10px' }}>
                Random Forest Analysis Skipped
              </p>
              <p style={{ fontSize: '14px', color: '#166534', marginBottom: '15px' }}>
                This application is on your whitelist and has been classified as clean (100% confidence).
              </p>
              <div className="feedback-section">
                <h4>Whitelist Feedback:</h4>
                <ul>
                  {classificationResults.feedback.map((item, index) => (
                    <li key={index} style={{ color: '#15803d' }}>{item}</li>
                  ))}
                </ul>
              </div>
            </div>
  ...
  const renderFinalResults = () => classificationResults && (
    <div className="result-section">
      ...
      <div className="action-buttons">
        <button className="btn primary" onClick={generateReport}>📄 Generate Complete Report</button>
